...
public class Arrays1 {
	...
	public static void main(String[] args) {
		...
		numbers2[0] = 100; // 주소값이 복사되어 같기 때문에 numbers2를 변경하면 numbers도 바뀜 
		printArray(numbers);
		printArray(numbers2);
		...
		// 5. sort
		Arrays.sort(numbers4); // 오름차순 정렬
		printArray(numbers4);
		
		int num = 10;
		Integer[] numArray = {1, 2, 3, 4, 5}; // 둘 다 정수형이지만 Collections를 사용하기 위해서는 Integer 자료형을 사용하여야한다.
		
		Arrays.sort(numArray, Collections.reverseOrder()); // 내림차순 정렬
		printArray(numArray);
		...
		// 1. binarySearch - 값의 위치 찾기 > 를 하기 위해서는 정렬을 먼저 해줘야 함
		Arrays.sort(numbers);
